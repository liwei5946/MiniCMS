	...
	<div id="container">
		...
		<table width="98%" border="0" align="center">
            <c:forEach var="mynews" items="${pageScope.news}" varStatus="countItem">
              <tr>
                <td><img src="img/bluearrow.gif" style="margin:2px;"/>
                <a href="SelectArticle?aid=${mynews.newsId}" title="${mynews.title}" target="_blank">${mynews.shortTitle}</a>                </td>
                <td align="right">${mynews.shorttime}</td>
              </tr>
            </c:forEach>
            <tr>
                ...
                <c:if test="${pageScope.nowPage != 1}">
                    <a href="list_article.jsp?tID=<%=type%>&jump=1">首页</a>
                    &nbsp;
                    <a href="list_article.jsp?tID=<%=type%>&jump=${pageScope.nowPage - 1}">上一页</a>
                    &nbsp;
                </c:if>
                ...
                <c:if test="${pageScope.nowPage != pageScope.pageCount}">
                    <a href="list_article.jsp?tID=<%=type%>&jump=${pageScope.nowPage + 1}">下一页</a>
                    &nbsp;
                    <a href="list_article.jsp?tID=<%=type%>&jump=${pageScope.pageCount}">尾页</a> 
                </c:if>
